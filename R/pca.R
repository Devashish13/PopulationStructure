...
PCA_GD <- function(data_path, metadata_path,geno_format = "pfile",
  ...
  df <- read.csv(metadata_path,check.names = FALSE,...)
...
PCA_plot <- function(pcadata_path,metadata_path,x = 1,y = 2,
                     sampleid_var = "#IID",color_var = "population",
                     shape_var = "region",color_vec = NULL,shape_vec= NULL,
                     figfile = "PCA_plot",...){
  ...
  df <- read.csv(metadata_path,check.names = F,...)
  ...
  if (!is.null(shape_var)){
    ...
    col_shape_df$shape_vec <- shape_vec[as.character(col_shape_df$region)]
  }
  ...
  PC_summary_df <-  data.frame(pca_eig_vec_2 %>%
                                group_by(!!sym(shape_var), !!sym(color_var)) %>%
                                 summarise(across(.cols = c(paste0("EV", x), paste0("EV", y)), .fns = mean)))
  } else if(is.null(shape_var) & !is.null(color_var)){
    PC_summary_df <-  data.frame(pca_eig_vec_2 %>%
  ...
  if (!is.null(shape_var)){
    ...
    shape_df = PC_summary_df[,shape_var]
  }
  ...
  p <- ggplot(PC_summary_df,aes(x=EV_x1 ,y=EV_y1,fill=PC_summary_df[,color_var],shape = shape_df)) +
    geom_point(...) +
    xlab(paste("PC",x,"(", pca_eig_val_pt[x],"%)",sep=" ")) +
    ylab(paste("PC",y,"(", pca_eig_val_pt[y],"%)",sep=" ")) +
    theme(legend.title=element_text(size=20), legend.text=element_text(size=18),
  ...
  dev.off()
    ...
    p <- ggplot(PC_summary_df,aes(x=EV_x1 ,y=EV_y1,fill=PC_summary_df[,color_var])) +
      geom_point(shape = 21,...) +
      xlab(paste("PC",x,"(", pca_eig_val_pt[x],"%)",sep=" ")) +
      ylab(paste("PC",y,"(", pca_eig_val_pt[y],"%)",sep=" ")) +
      theme(legend.title=element_text(size=20), legend.text=element_text(size=18),
...
PCA_outliers <- function(pcadata_path,metadata_path,color_var,sampleid_var,
